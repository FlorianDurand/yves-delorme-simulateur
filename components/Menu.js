...
const Menu = props => {
	...
	useEffect(() => {
		...
		{
			setArrayItem(flatSheet)
		}
		if(props.typeItem == "fittedSheet")
		{
			setArrayItem(fittedSheet)
		}

		if(props.typeItem == "bigPillow")
		{
			setArrayItem(bigPillow)
		}
	},[])
	...
	return(
		<div className={styles.menu}>
			...
			<div className={styles.articles}>
				{arrayItem.map(article=>
					<Article idActiveArticle={itemId(typeItem)} onArticleChange={onIdChange} onItemChange={props.onItemChange}  typeItem={typeItem} {...article }  key={article.name} />
				)} 
			</div>					
		</div>	
	)
